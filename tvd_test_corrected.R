...
tvd_test <- function(df, value_col = "MP", dir_col = "Dir", bin_col = "Bin", n_perm = 10000) {
  ...
  unique_bins <- sort(unique(df[[bin_col]]))
  ...
  results_list <- lapply(unique_bins, function(bin_val) {
    ...
    if (length(group_vals) != 2) {
      ...
      return(data.frame(Bin = bin_val, TVD = NA, p_value = NA, p_value_BH = NA, Larger_Dir = NA))
    }
    ...
    perm_tvd <- replicate(n_perm, {
      perm_labels <- sample(rep(group_vals, c(n1, n2)))
  ...
  valid_idx <- !is.na(res$p_value)
  res$p_value_BH <- NA
  res$p_value_BH[valid_idx] <- p.adjust(res$p_value[valid_idx], method = "BH")
  res
}

process_one_file <- function(filename, bin_size = 10, bound = 2000) {
  assembly_name <- sub("^(MP_transcriptTSS_|MP_CpGTSS_|MP_nonCpGTSS_)(.*?)\\.tsv$", "\\2", filename)

  tvd_data <- fread(filename)[V2 <= bound & V2 >= (-bound), ]
  ...
  left <- tvd_data[tvd_data$Dir == "5'"]
  right <- tvd_data[tvd_data$Dir == "3'"]

  left <- data.frame(summarise(group_by(left, Bin),
                               Mean = mean(MP),
                               CI_lower = ci(MP)[2],
                               CI_upper = ci(MP)[3]))
  right <- data.frame(summarise(group_by(right, Bin),
                                Mean = mean(MP),
                                CI_lower = ci(MP)[2],
                                CI_upper = ci(MP)[3]))

  out_tvd_data <- data.frame()
  out_tvd_data <- rbind(out_tvd_data, cbind(left, Dir = "5'"))
  out_tvd_data <- rbind(out_tvd_data, cbind(right, Dir = "3'"))
  out_tvd_data <- out_tvd_data[out_tvd_data$Bin <= bound, ]

  prefix <- if (grepl("^MP_transcriptTSS_", filename)) {
    "transcript"
  } else if (grepl("^MP_CpGTSS_", filename)) {
    "CpG"
  } else {
    "nonCpG"
  }
...
files <- list.files(pattern = "^(MP_transcriptTSS_|MP_CpGTSS_|MP_nonCpGTSS_).*\\.tsv$")
